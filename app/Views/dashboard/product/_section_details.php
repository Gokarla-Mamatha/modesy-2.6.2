                                ...
                                if (!empty($customField)):
                                    if ($customField->field_type == 'text'):
                                        ...
                                        <div class="col-sm-12 col-sm-6 col-custom-field">
                                            ...
                                            <input type="text" name="field_<?= $customField->id; ?>" class="form-control form-input" value="<?= esc($inputValue); ?>" placeholder="<?= esc($customField->name); ?>" <?= $customField->is_required == 1 ? 'required' : ''; ?> data-type="title">
                                        </div>
                                    <?php elseif ($customField->field_type == 'number'):
                                        $inputValue = getProductCustomFieldInputValue($customField->id, $product->id); ?>
                                        <div class="col-sm-12 col-sm-6 col-custom-field">
                                            <label><?= esc($customField->name); ?><?= $customField->is_required != 1 ? ' (' . trans("optional") . ')' : ''; ?></label>
                                            <input type="number" name="field_<?= $customField->id; ?>" class="form-control form-input" value="<?= esc($inputValue); ?>" placeholder="<?= esc($customField->name); ?>" min="0" max="999999999" <?= $customField->is_required == 1 ? 'required' : ''; ?> data-type="number">
                                        </div>
                                    <?php elseif ($customField->field_type == 'textarea'):
                                        $inputValue = getProductCustomFieldInputValue($customField->id, $product->id); ?>
                                        <div class="col-sm-12 col-sm-6 col-custom-field">
                                            <label><?= esc($customField->name); ?><?= $customField->is_required != 1 ? ' (' . trans("optional") . ')' : ''; ?></label>
                                            <textarea class="form-control form-input custom-field-input" name="field_<?= $customField->id; ?>" placeholder="<?= esc($customField->name); ?>" <?= $customField->is_required == 1 ? 'required' : ''; ?> style="min-height: 80px;" data-type="text" >@esc($inputValue)</textarea>
                                        </div>
...
<?php endif; ?>
<?php if (isset($product) && $product->product_type == 'coupon'): ?>
    <?php $couponEditorId = 'coupon_editor'; ?>
    <div class="section-product-details">
        <div class="form-box form-box-last" style="padding-bottom: 0;">
            <div class="form-box-head">
            ...
            <div class="form-box-body">
                ...
                <div class="form-group m-b-15">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <button type="button"
                                    id="btn_add_image_editor"
                                    class="btn btn-md btn-info m-b-10"
                            ...
                            <?php if (aiWriter()->status && hasPermission('ai_writer')): ?>
                                <button type="button"
                                        class="btn btn-md btn-default btn-open-ai-writer m-b-10"
                                        data-toggle="modal"
                    ...
                    <textarea name="coupon_description"
                              id="<?= $couponEditorId; ?>"
                             data-type="text" class="tinyMCE text-editor"><?= esc($product->coupon_description ?? ''); ?></textarea>
                </div>
