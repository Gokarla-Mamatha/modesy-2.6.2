                                    ...
                                    <table role="presentation" border="0" cellpadding="0" cellspacing="0" style="text-align: left" class="table-products">
                                        ...
                                        <?php if (!empty($orderProducts)):
                                            foreach ($orderProducts as $item): ?>
                                                ...
                                                $assignedCoupons = $db->table('variant_coupons')
                                                    ->where('order_item_id', $item->id)   // ✔ CORRECT
                                                    ->get()
                                                    ->getResult();

                                                if ($item->product_type === 'coupon' && !empty($assignedCoupons)): ?>
                                                    <tr>
                                                        <td colspan="5" style="padding: 12px; background: #f7f7f7; border-bottom: 1px solid #ddd;">
                                                            <strong>Your Coupon Codes:</strong><br>
                                                            <?php foreach ($assignedCoupons as $cp): ?>
                                                                <div style="margin-top: 6px; font-size: 14px;">
                                                                    <strong><?= esc($cp->coupon_code); ?></strong>
                                                                </div>
                                                            <?php endforeach; ?>
                                                        </td>
                                                    </tr>
