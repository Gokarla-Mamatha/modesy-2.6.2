<div class="wallet-container">
    <div class="d-flex justify-content-end">
        <button type="button" class="btn btn-md btn-custom m-b-15" data-toggle="modal" data-target="#modalPayoutRequest">
            ...
            </svg>&nbsp;<?= esc(trans("new_payout_request")); ?>
        </button>
    </div>
    <div class="table-responsive table-custom">
        <table class="table" role="grid">
            <thead>
            <tr role="row">
                <th scope="col"><?= esc(trans("withdraw_method")); ?></th>
                <th scope="col"><?= esc(trans("withdraw_amount")); ?></th>
            ...
            <?php if (!empty($payouts)):
                foreach ($payouts as $payout): ?>
                    <tr>
                        <td><?= esc(trans($payout->payout_method)); ?></td>
                        ...
                        <td>
                            ...
                            <?php else: ?>
                                <span class="badge badge-secondary-light"><?= esc(trans("pending")); ?></span>
                            <?php endif; ?>
                        </td>
        ...
        <div class="modal-content modal-custom modal-refund">
            <form action="<?= base_url('wallet/new-payout-request-post'); ?>" method="post">
                ...
                <div class="modal-header">
                    <h5 class="modal-title"><?= esc(trans("new_payout_request")); ?></h5>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="new-payout-request">
                                <div class="form-group">
                                    <label class="control-label"><?= esc(trans("withdraw_amount")); ?></label>
                                    <?php $minValue = 0;
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" name="amount" class="form-control form-input input-price" maxlength="13" placeholder="<?= $defaultCurrency->currency_format == 'european' ? '0,00' : '0.00'; ?>"
                                               placeholder="<?= $defaultCurrency->currency_format == 'european' ? '0,00' : '0.00'; ?>" inputmode="decimal" required>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="control-label"><?= esc(trans("withdraw_method")); ?></label>
                                    <select name="payout_method" class="form-control custom-select" onchange="update_payout_input(this.value);" required>
                                        ...
                                        if (!empty($payoutOptions)):
                                            foreach ($payoutOptions as $option):?>
                                                <option value="<?= $option; ?>"><?= esc(trans($option)); ?></option>
                                            <?php endforeach;
                                        endif; ?>
                                    </select>
                                </div>
                        ...
                        <div class="col-12">
                            <div class="minimum-payout-container">
                                <div class="font-700 m-b-15"><?= esc(trans("min_poyout_amounts")); ?></div>
                                <?php if ($paymentSettings->payout_paypal_enabled): ?>
                                    <span><?= esc(trans("paypal")); ?></span>:&nbsp;<strong><?= priceFormatted($paymentSettings->min_payout_paypal, $paymentSettings->default_currency) ?></strong>&nbsp;&nbsp;
                                <?php endif;
                                if ($paymentSettings->payout_bitcoin_enabled): ?>
                                    <span><?= esc(trans("bitcoin")); ?></span>:&nbsp;<strong><?= priceFormatted($paymentSettings->min_payout_bitcoin, $paymentSettings->default_currency) ?></strong>&nbsp;&nbsp;
                                <?php endif;
                                if ($paymentSettings->payout_iban_enabled): ?>
                                    <span><?= esc(trans("iban")); ?></span>:&nbsp;<strong><?= priceFormatted($paymentSettings->min_payout_iban, $paymentSettings->default_currency) ?></strong>&nbsp;&nbsp;
                                <?php endif;
                                if ($paymentSettings->payout_swift_enabled): ?>
                                    <span><?= esc(trans("swift")); ?></span>:&nbsp;<strong><?= priceFormatted($paymentSettings->min_payout_swift, $paymentSettings->default_currency) ?></strong>
                                <?php endif; ?>
                                <hr>
                                <strong><?= esc(trans("your_balance")); ?>:&nbsp;<span class="balance text-success"><?= priceFormatted(user()->balance, $paymentSettings->default_currency) ?></span></strong>
                            </div>
                        </div>
                    </div>
                </div>
