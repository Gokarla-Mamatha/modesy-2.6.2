            ...
            <div class="col-right">
                <ul class="navbar-nav clearfix">
                    ...
                    <li class="nav-item">
                        ...
                        <button type="button"
                            ...
                            class="nav-link btn-modal-location button-link btn-modal-location-header display-flex align-items-center"
                            ...
                            &nbsp;
                               <?php
                               $manualLocation = session()->get('manual_location');

                                if (!empty($manualLocation['location_text'])) {
                                    echo esc($manualLocation['location_text']);
                                } elseif (!empty($baseVars->defaultLocationInput)) {
                                    echo esc($baseVars->defaultLocationInput);
                                } else {
                                    echo trans('location');
                                }
                                ?>
                        </button>
                        <?php if (!empty($manualLocation['location_text']) || !empty($baseVars->defaultLocationInput)): ?>
                    ...
                    <li class="nav-item dropdown top-menu-dropdown">
                        <button type="button" class="nav-link dropdown-toggle button-link" data-bs-toggle="dropdown"
                    ...
                    <li class="nav-item dropdown top-menu-dropdown">
                        <button type="button" class="nav-link dropdown-toggle button-link" data-bs-toggle="dropdown"
                            ...
                            <img src="<?= base_url($activeLang->flag_path) ?>" class="flag"
                                style="width: 18px; height: auto;"
                                alt="<?= esc($activeLang->name) . ' ' . trans('active') ?>"><?= esc($activeLang->name) ?>&nbsp;<i
                                class="icon-arrow-down"></i>
                        </button>
                        <ul class="dropdown-menu dropdown-menu-lang">
                            ...
                            <li>
                                <a href="<?= convertUrlByLanguage($language) ?>"
                                    ...
                                    <img src="<?= base_url($language->flag_path) ?>" class="flag"
                                        style="width: 18px; height: auto;"
                                        alt="<?= esc($language->name) ?>"><?= esc($language->name) ?>
                                </a>
                            </li>
                            <?php endforeach; ?>
                        </ul>
                ...
                if (authCheck()): ?>
                    <li class="nav-item dropdown profile-dropdown p-r-0">
                        <button type="button" class="nav-link dropdown-toggle a-profile button-link"
                            data-bs-toggle="dropdown" aria-expanded="false" aria-label="select-language">
                            <img src="<?= getUserAvatar(user()->avatar, user()->storage_avatar) ?>"
                                alt="<?= esc(getUsername(user())) ?>" width="26" height="26">
