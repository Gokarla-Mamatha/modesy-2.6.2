                            ...
                            <ul id="navbar_mobile_links" class="navbar-nav">
                                <?php if (authCheck()): ?>
                                    <li class="dropdown profile-dropdown nav-item">
                                        <a href="#" class="dropdown-toggle image-profile-drop nav-link" data-bs-toggle="dropdown" aria-expanded="false">
                                ...
                                if ($generalSettings->location_search_header == 1 && countItems($activeCountries) > 0): ?>
                                    <li class="nav-item nav-item-messages">
                                        <button type="button" data-bs-toggle="modal" data-bs-target="#locationModal" class="nav-link btn-modal-location close-menu-click button-link" aria-label="nav-location-menu">
                                ...
                                <li class="d-flex justify-content-center mobile-flex-dropdowns">
                                    <?php if ($generalSettings->multilingual_system == 1 && countItems($activeLanguages) > 1): ?>
                                        <div class="nav-item dropdown top-menu-dropdown">
                                            <button type="button" class="nav-link dropdown-toggle button-link" data-bs-toggle="dropdown" aria-label="nav-flag-menu">
                                    ...
                                    if ($paymentSettings->currency_converter == 1 && countItems($currencies) > 1): ?>
                                        <div class="nav-item dropdown top-menu-dropdown">
                                            <button type="button" class="nav-link dropdown-toggle button-link" data-bs-toggle="dropdown" aria-label="nav-currency-menu">
...
function handleSellNowClick() {
    ...
    <?php if (!authCheck()): ?>
        ...
        window.location.href = "<?= generateUrl('register'); ?>";
    <?php else: ?>
        window.location.href = "<?= generateDashUrl('add_product'); ?>";
    <?php endif; ?>
