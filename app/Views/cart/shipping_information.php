                                                            ...
                                                            <div class="col-12 m-b-10">
                                                                <div class="shipping-address-box shipping-address-box-cart">
                                                                    ...
                                                                    <div class="profile-actions-shipping profile-actions-shipping-cart">
                                                                        <a href="#" data-bs-toggle="modal" data-bs-target="#modalAddress<?= $address->id; ?>">
                                                                    ...
                                                                    <div class="shipping-address-box shipping-address-box-cart">
                                                                        ...
                                                                        <div class="profile-actions-shipping profile-actions-shipping-cart">
                                                                            ...
                                                                            <!-- <a href="javascript:void(0)" onclick='deleteShippingAddress("<?= $address->id; ?>","<?= trans("confirm_delete", true); ?>");'> -->
                                                                                <a href="#" class="btn-delete-shipping" data-id="<?= $address->id; ?>" data-msg="<?= trans('confirm_delete', true); ?>">
                                                                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="#777777" class="bi bi-trash3" viewBox="0 0 16 16">
                                                                                    <path d="M6.5 1h3a.5.5 0 0 1 .5.5v1H6v-1a.5.5 0 0 1 .5-.5ZM11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3A1.5 1.5 0 0 0 5 1.5v1H2.506a.58.58 0 0 0-.01 0H1.5a.5.5 0 0 0 0 1h.538l.853 10.66A2 2 0 0 0 4.885 16h6.23a2 2 0 0 0 1.994-1.84l.853-10.66h.538a.5.5 0 0 0 0-1h-.995a.59.59 0 0 0-.01 0H11Zm1.958 1-.846 10.58a1 1 0 0 1-.997.92h-6.23a1 1 0 0 1-.997-.92L3.042 3.5h9.916Zm-7.487 1a.5.5 0 0 1 .528.47l.5 8.5a.5.5 0 0 1-.998.06L5 5.03a.5.5 0 0 1 .47-.53Zm5.058 0a.5.5 0 0 1 .47.53l-.5 8.5a.5.5 0 1 1-.998-.06l.5-8.5a.5.5 0 0 1 .528-.47ZM8 4.5a.5.5 0 0 1 .5.5v8.5a.5.5 0 0 1-1 0V5a.5.5 0 0 1 .5-.5Z"/>
                                                                                </svg>
                                                                            </a>
...
<div id="modalAddAddress" class="modal fade modal-custom" tabindex="-1" data-bs-focus="false" role="dialog">
                        ...
                        <div class="row">
                            <?php if ($generalSettings->single_country_mode != 1): ?>
                                <div class="col-12 col-md-6 m-b-sm-15">
                                    ...
                                    <select id="select_countries_new_address" name="country_id" class="select2 select2-req form-control" data-placeholder="<?= trans("country"); ?>" required>
                                        <option></option>
...
<?php if (!empty($shippingAddresses)):
    foreach ($shippingAddresses as $address):?>
        <div id="modalAddress<?= $address->id; ?>" class="modal fade modal-custom" role="dialog" tabindex="-1" data-bs-focus="false">
            <div class="modal-dialog modal-dialog-shipping-address">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal"><i class="icon-close"></i></button>
                            ...
                            <div class="form-group">
                                <div class="row">
                                    <?php if ($generalSettings->single_country_mode != 1): ?>
                                        <div class="col-12 col-md-6 m-b-sm-15">
                                            <label class="control-label"><?= trans("country"); ?></label>
                                            <select id="select_countries_address_<?= $address->id; ?>" name="country_id" class="select2 select2-req form-control select-countries-edit-address" data-address-id="<?= $address->id; ?>" required>
                                                <?php if (!empty($activeCountries)):
                                                    foreach ($activeCountries as $item): ?>
...
document.addEventListener('DOMContentLoaded', function () {
    $('#modalAddAddress').on('shown.bs.modal', function () {
        fetch('<?= base_url('csrf-token'); ?>')
            ...
            .then(data => {
                const form = document.getElementById('form_add_shipping_address');
                const input = form.querySelector('input[name="' + data.name + '"]');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        fetch('/csrf-token')
            .then(response => response.json())
            .then(csrf => {
                const input = form.querySelector('input[name="' + csrf.name + '"]');
