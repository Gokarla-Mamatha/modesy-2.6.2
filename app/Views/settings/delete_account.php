                        ...
                        <form action="<?= base_url('delete-account-post'); ?>" method="post" id="form_validate">
                            ...
                            <?php if (user()->account_delete_req == 1): ?>
                                <div class="alert alert-info" role="alert">
                                    <?= esc(trans("delete_account_submit_exp")); ?>
                                </div>
                            <?php else: ?>
                                <div class="alert alert-danger" role="alert">
                                    <?= esc(trans("delete_account_exp")); ?>
                                </div>
                                <div class="form-group">
                                    <label class="control-label"><?= esc(trans("password")); ?></label>
                                    <input type="password" name="password" class="form-control form-input" value="<?= old("password"); ?>" placeholder="<?= esc(trans("password")); ?>" minlength="4" maxlength="255" required>
                                </div>
                                <button type="submit" name="submit" value="submit" class="btn btn-md btn-danger text-white m-t-10">
