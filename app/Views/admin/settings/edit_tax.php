    ...
    <div class="col-sm-12 col-md-8">
        <div class="box box-primary">
            <div class="box-header with-border">
                <div class="left">
                    <h3 class="box-title"><?= esc(trans('edit_tax')); ?></h3>
                </div>
                <div class="right">
                    <a href="<?= adminUrl("payment-settings"); ?>" class="btn btn-success btn-add-new">
                        <i class="fa fa-list-ul"></i>&nbsp;&nbsp;<?= esc(trans('payment_settings')); ?>
                    </a>
                </div>
            </div>
            <form action="<?= base_url('Admin/editTaxPost'); ?>" method="post" enctype="multipart/form-data">
                ...
                <div class="box-body">
                    <div class="form-group">
                        <label><?= esc(trans("tax_name")); ?></label>
                        <?php foreach ($activeLanguages as $language):
                            $nameArray = !empty($tax->name_data) ? unserializeData($tax->name_data) : null; ?>
                            <input type="text" name="tax_name_<?= $language->id; ?>" value="<?= esc(getTaxName($nameArray, $language->id)); ?>" class="form-control m-b-5" placeholder="<?= esc($language->name); ?>" maxlength="255" data-type="text" required>
                        <?php endforeach; ?>
                    </div>
                    <div class="form-group">
                        <label><?= esc(trans('tax_rate')); ?>(%)</label>
                        <input type="number" name="tax_rate" class="form-control" min="0" max="100" step="0.01" value="<?= esc($tax->tax_rate); ?>" data-type="decimal" required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <div class="custom-control custom-checkbox" style="margin-bottom: 20px;">
                            ...
                            <label for="checkboxAllCountries" class="custom-control-label"><?= esc(trans("all_locations")); ?></label>
                        </div>
