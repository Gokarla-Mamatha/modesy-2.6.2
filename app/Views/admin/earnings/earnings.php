                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped" role="grid">
                        ...
                        <tr role="row">
                            ...
                            <th><?= esc(trans("vat")); ?></th>
                            <th><?= esc(trans("commissions_discounts")); ?></th>
                            <th><?= esc(trans('shipping_cost')); ?></th>
                            <th><?= esc(trans('earned_amount')); ?></th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="font-size-13">
                                            <?= esc(trans("commission")); ?>:&nbsp;<span class="text-danger"><?= priceFormatted($item->commission, $item->currency); ?>&nbsp;<?= !empty($item->commission_rate) ? '(' . $item->commission_rate . '%)' : ''; ?></span>
                                        </div>
                                        <?php if (!empty($item->affiliate_commission)): ?>
                                            <div class="font-size-13 m-t-5">
                                                <?= esc(trans("referrer_commission")); ?>:&nbsp;<span class="text-danger"><?= priceFormatted($item->affiliate_commission, $item->currency); ?>&nbsp;(<?= $item->affiliate_commission_rate; ?>%)</span>
                                            </div>
                                        <?php endif; ?>
                                        <?php if (!empty($item->affiliate_discount)): ?>
                                            <div class="font-size-13 m-t-5">
                                                <?= esc(trans("referral_discount")); ?>:&nbsp;<span class="text-danger"><?= priceFormatted($item->affiliate_discount, $item->currency); ?>&nbsp;(<?= $item->affiliate_discount_rate; ?>%)</span>
                                            </div>
                                        <?php endif; ?>
                                        <?php if (!empty($item->coupon_discount)): ?>
                                            <div class="font-size-13 m-t-5">
                                                <?= esc(trans("discount_coupon")); ?>:&nbsp;<span class="text-danger"><?= priceFormatted($item->coupon_discount, $item->currency); ?></span>
                                    ...
                                    <td>
                                        ...
                                        if (!empty($order) && $order->payment_method == 'cash_on_delivery'):?>
                                            <span class="text-danger">(-<?= priceFormatted($item->earned_amount, $item->currency); ?>)</span><br><small class="text-danger"><?= esc(trans("cash_on_delivery")); ?></small>
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu options-dropdown">
                                                <li>
                                                <!-- <a href="javascript:void(0)" onclick="deleteItem('Earnings/deleteEarningPost','<?= $item->id; ?>','<?= esc(trans("confirm_delete", true)); ?>');"> -->
                                                    <a href="#" class="btn-item-delete" data-url="Earnings/deleteEarningPost" data-id="<?= $item->id; ?>" data-msg="<?= esc(trans('confirm_delete', true)); ?>">
