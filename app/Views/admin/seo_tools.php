...
<div class="row">
    <div class="col-lg-6 col-md-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title"><?= esc(trans('sitemap')); ?></h3>
            </div>
            <form action="<?= base_url('Admin/generateSitemapPost'); ?>" method="post">
                <?= csrf_field(); ?>
                <div class="box-body">
                    <div class="form-group">
                        <label class="label-sitemap"><?= esc(trans('frequency')); ?></label>
                        <small class="small-sitemap"> (<?= esc(trans('frequency_exp')); ?>)</small>
                        <?= formRadio('frequency', 'auto', 'none', trans("automatically_calculated"), trans("none"), $productSettings->sitemap_frequency); ?>
                    </div>
                    <div class="form-group">
                        <label class="label-sitemap"><?= esc(trans('last_modification')); ?></label>
                        <small class="small-sitemap"> (<?= esc(trans('last_modification_exp')); ?>)</small>
                    ...
                    <div class="form-group">
                        <label class="label-sitemap"><?= esc(trans('priority')); ?></label>
                        <small class="small-sitemap"> (<?= esc(trans('priority_exp')); ?>)</small>
                        <?= formRadio('priority', 'auto', 'none', trans("automatically_calculated"), trans("none"), $productSettings->sitemap_priority); ?>
                    </div>
                </div>

                <div class="box-footer">
                    <button type="submit" class="btn btn-primary pull-right"><?= esc(trans('generate_sitemap')); ?></button>
                </div>
            </form>
            ...
            if (!empty($files)): ?>
                <div style="padding: 20px">
                    <h3 style="font-size: 18px; font-weight: 500;"><?= esc(trans("generated_sitemaps")) ?></h3>
                    <hr>
        ...
        <div class="callout" style="margin-top: 30px;background-color: #fff; border-color:#00c0ef;max-width: 600px;">
            ...
            <small><?= esc(trans('msg_cron_sitemap')); ?></small>
        </div>
    </div>
    <form action="<?= base_url('Admin/seoToolsPost'); ?>" method="post">
        ...
        <div class="col-lg-6 col-md-12">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title"><?= esc(trans('google_analytics')); ?></h3>
                </div>
                <div class="box-body">
                    <div class="form-group">
                        <textarea class="form-control text-area" name="google_analytics" placeholder="<?= esc(trans('google_analytics')); ?>" style="min-height: 100px;" data-type="text"><?= esc($generalSettings->google_analytics); ?></textarea>
                    </div>
