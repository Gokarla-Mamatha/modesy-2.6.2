                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped" role="grid">
                        ...
                        <tr role="row">
                            ...
                            <th scope="col"><?= esc(trans("seller")); ?></th>
                                        ...
                                        <td>
                                            <form action="<?= base_url('OrderAdmin/approveRefundPost'); ?>" method="post">
                                                ...
                                                <?php if ($request->is_completed == 1): ?>
                                                    <a href="<?= adminUrl('refund-requests/' . $request->id); ?>" class="btn btn-sm btn-default btn-edit"><i class="fa fa-info-circle" aria-hidden="true"></i>&nbsp;<?= esc(trans("details")); ?></a>
                                                    <label class="label label-success"><i class="fa fa-check" aria-hidden="true"></i>&nbsp;<?= esc(trans("refund_approved")); ?></label>
                                                <?php else: ?>
                                                    <div class="btn-group btn-group-option">
                                                        <a href="<?= adminUrl('refund-requests/' . $request->id); ?>" class="btn btn-sm btn-default btn-edit"><i class="fa fa-info-circle" aria-hidden="true"></i>&nbsp;<?= esc(trans("details")); ?></a>
                                                        <button type="submit" class="btn btn-sm btn-default btn-edit"><i class="fa fa-check" aria-hidden="true"></i>&nbsp;<?= esc(trans("approve_refund")); ?></button>
                                                    </div>
                                                <?php endif; ?>
                                            </form>
                                        </td>
...
<div class="row">
    <div class="col-sm-9">
        <div class="alert alert-info alert-large m-t-10">
            <strong><?= esc(trans("warning")); ?>!</strong>&nbsp;&nbsp;<?= esc(trans("refund_admin_complete_exp")); ?>
        </div>
    </div>
</div>