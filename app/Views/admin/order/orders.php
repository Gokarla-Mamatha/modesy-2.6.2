                            ...
                            <form action="<?= adminUrl('orders'); ?>" method="get" id="formFilterOrders">
                                ...
                                <div class="item-table-filter">
                                    ...
                                    <select name="status" class="form-control">
                                        ...
                                        <option value="processing" <?= inputGet('status') == 'processing' ? 'selected' : ''; ?>><?= esc(trans("order_processing")); ?></option>
                                        <option value="completed" <?= inputGet('status') == 'completed' ? 'selected' : ''; ?>><?= esc(trans("completed")); ?></option>
                                        <option value="cancelled" <?= inputGet('status') == 'cancelled' ? 'selected' : ''; ?>><?= esc(trans("cancelled")); ?></option>
                                    </select>
                                </div>
                                <div class="item-table-filter">
                                    ...
                                    <div class="item-table-filter-search">
                                        ...
                                        <div class="btn-group table-export">
                                            ...
                                            <ul class="dropdown-menu" role="menu">
                                                ...
                                                <li>
                                                    <button type="button" class="btn-export-data" data-export-form="formFilterOrders" data-export-type="orders" data-export-file-type="excel"><?= esc(trans("excel")); ?>&nbsp;(.xlsx)</button>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
                        ...
                        <?php if (!empty($orders)):
                            foreach ($orders as $item): ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($item->buyer_id == 0): ?>
                                            <div class="table-orders-user">
                                                ...
                                                <?php endif; ?>
                                                <label class="label bg-olive label-order-guest"><?= esc(trans("guest")); ?></label>
                                            </div>
                                        <?php else: ?>
                                            <div class="table-orders-user">
                                    ...
                                    <td>
                                        <div class="m-b-5">
                                            <?= esc(trans($item->payment_status)); ?>
                                        </div>
                                        <a href="<?= base_url('invoice/' . $item->order_number . '?type=admin'); ?>" class="btn btn-sm btn-default" target="_blank"><i class="fa fa-file-text"></i>&nbsp;&nbsp;<?= esc(trans("view_invoice")); ?></a>
                                    </td>
                                        ...
                                        <form action="<?= base_url('OrderAdmin/orderPaymentReceivedPost'); ?>" method="post">
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu options-dropdown" style="min-width: 190px;">
                                                    <li><a href="<?= adminUrl('order-details/' . $item->id); ?>"><i class="fa fa-info option-icon"></i><?= esc(trans('view_details')); ?></a></li>
                                                    <?php if ($item->status != 2):
                                                        if ($item->payment_status != 'payment_received'): ?>
                                                            <li>
                                                                <button type="submit" name="option" value="payment_received" class="btn-list-button"><i class="fa fa-check option-icon"></i><?= esc(trans('payment_received')); ?></button>
                                                            </li>
                                                            <li><a href="javascript:void(0)" onclick='cancelOrder(<?= $item->id; ?>,"<?= esc(trans("confirm_action", true)); ?>");'><i class="fa fa-ban option-icon"></i><?= esc(trans('cancel_order')); ?></a></li>
                                                        <?php endif; ?>
                                                    <?php endif; ?>
                                                    <li>
                                                        <!-- <a href="javascript:void(0)" onclick="deleteItem('OrderAdmin/deleteOrderPost','<?= $item->id; ?>','<?= esc(trans("confirm_delete", true)); ?>');"> -->
                                                            <a href="#" class="btn-item-delete" data-url="OrderAdmin/deleteOrderPost" data-id="<?= $item->id; ?>" data-msg="<?= esc(trans('confirm_user', true)); ?>">
