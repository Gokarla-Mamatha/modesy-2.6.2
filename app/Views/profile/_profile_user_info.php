<div class="row-custom">
    <div class="profile-details">
        ...
        <div class="right">
           ...
           <?php if (authCheck() && user()->id == $user->id && !empty($user->referral_code)): ?>
                <div class="row-custom m-t-10">
                    <div style="border:1px dashed #dbeafe;background:#f8fbff;padding:12px;border-radius:8px;max-width:420px;">
                        <div style="font-size:14px;font-weight:600;color:#1e3a8a;margin-bottom:4px;">
                            🎁 Invite Friends & Earn Rewards
                        </div>

                        <div style="font-size:13px;margin-bottom:6px;">
                            Code: <strong><?= esc($user->referral_code); ?></strong>
                        </div>
                        <div style="display:flex;gap:6px;">
                            <input id="referralLink" type="text"
                                value="<?= base_url('register?ref=' . $user->referral_code); ?>" readonly
                                style="flex:1;padding:6px 8px;font-size:13px;border:1px solid #ccc;border-radius:4px;">
                            <button id="copyBtn"
                                    onclick="copyReferralLink()"
                                    style="padding:6px 14px;font-size:13px;border:0;background:#2563eb;color:#fff;border-radius:4px;cursor:pointer;">
                                Copy
                            </button>
                        </div>
                    </div>
               ...
               <script <?= csp_script_nonce() ?>>
                    function copyReferralLink(){
                        const i = document.getElementById('referralLink'),
                            b = document.getElementById('copyBtn');
