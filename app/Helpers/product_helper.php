...
// get new bid requests count
if (!function_exists('getNewProductBidRequestsCount')) {
    function getNewProductBidRequestsCount($sellerId)
    {
        $model = new \App\Models\BiddingModel();
        return $model->getNewProductBidRequestsCount($sellerId);
    }
}
...
if (!function_exists('clean_html')) {
    function clean_html($html)
    {
        ...
        $html = preg_replace('/<(iframe|object|embed)[^>]*>(.*?)<\/\1>/is', '', $html);

        $allowed_tags = '<p><br><b><strong><i><em><ul><ol><li><h1><h2><h3><h4><h5>';
...
if (!function_exists('get_tax_amount')) {
    ...
    function get_tax_amount($cart_items)
    {
        if (empty($cart_items)) {
            return 0;
        }

        $taxcontroller = new \App\Controllers\TaxCheckoutController();

        return $taxcontroller->tax($cart_items);
    }
}
