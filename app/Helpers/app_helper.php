...
if (!function_exists('isActiveAffiliateProduct')) {
    function isActiveAffiliateProduct($product, $vendor)
    {
        ...
        if ($affiliateSettings->status == 1 && !empty($product) && !empty($vendor)) {
            if (authCheck() && user()->is_affiliate == 1 && $product->listing_type != 'ordinary_listing' && $product->listing_type != 'bidding' && $product->listing_type != 'productbid' && $product->is_free_product != 1) {
                $status = true;
            }
...
function getUserAvatar($img = null)
{
