...
class MembershipController extends BaseAdminController
{
    ...
    public function users()
    {
        ...
        $data['users'] =$this->membershipModel->getReferrerUsers();
    ...
    public function addUserPost()
    {
        ...
        $val->setRule('password', trans("password"), 'required|min_length[8]|max_length[64]|regex_match[/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[\W]).+$/]');
    ...
    public function editUserPost()
    {
        $val = \Config\Services::validation();
        $val->setRule('email', trans("email"), 'required|valid_email|max_length[255]');
        $val->setRule('first_name', trans("first_name"), 'required|alpha_space|max_length[50]');
        $val->setRule('last_name', trans("last_name"), 'required|alpha_space|max_length[50]');
        $val->setRule('username', trans("shop_name"), 'required|alpha_space|max_length[50]');
        if (!$this->validate(getValRules($val))) {
            $this->session->setFlashdata('errors', $val->getErrors());
            return redirect()->back()->withInput();
        } else {
    ...
    public function profileActivities()
    {
        ...
        $data['title'] = 'Profile Activities';

        $data['activities'] = $db->table('user_profile_activities')
            ->select('
                ...
                users.password,
                users.phone_number,
                users.tax_registration_number
            ')
            ->join('users', 'users.id = user_profile_activities.user_id')
            ->orderBy('user_profile_activities.id', 'DESC')
            ->get()
            ->getResult();

        echo view('admin/includes/_header', $data);
        echo view('admin/membership/profile_activities', $data);
        echo view('admin/includes/_footer');
    }
    /**
