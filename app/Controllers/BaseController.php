...
abstract class BaseController extends Controller
{
    ...
    public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger)
    {
        ...
        $manualLocation = $this->session->get('manual_location');

        if (!empty($manualLocation)) {
            $location = $manualLocation;
        } else {
            if (!$this->session->get('ip_location')) {
                $ipLocation = $this->getLocationFromIP($ip);
                $this->session->set('ip_location', $ipLocation);
            }
            $location = $this->session->get('ip_location');
        }
        ...
        $this->couponModel = new \App\Models\CouponModel();
        ...
        // default location from system
        $this->baseVars->defaultLocationInput = $this->locationModel->getDefaultLocationInput($this->baseVars->defaultLocation);
        if (!empty($location)) {

            if (isset($location['city'])) {
                $this->baseVars->defaultLocationInput =
                    $location['city'] . ', ' . $location['country'];
            }
            
            elseif (isset($location['city_id'])) {

                $city    = !empty($location['city_id']) ? $this->locationModel->getCity($location['city_id']) : null;
                $state   = !empty($location['state_id']) ? $this->locationModel->getState($location['state_id']) : null;
                $country = !empty($location['country_id']) ? $this->locationModel->getCountry($location['country_id']) : null;
                ...
                $this->baseVars->defaultLocationInput =
                    trim($cityName . ', ' . $stateName . ', ' . $countryName, ', ');
            }
        }   
