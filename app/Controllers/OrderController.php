...
class OrderController extends BaseController
{
    ...
    public function placeBidPost()
    {
        $productId = inputPost('product_id');
        $amount    = inputPost('bid_amount');

        $product = $this->productModel->getActiveProduct($productId);
        $pageLink = '&nbsp;<a href="' . generateUrl("productbid_requests") . '" class="link-blue link-underlined">'
            . trans("Bids") . '</a>';
        ...
        $result = $this->biddingModel->placeBid($productId, $amount);

        if (!$result['success']) {
            $this->session->setFlashdata('product_details_error',$result['message']);
            return redirect()->back();
        }
        ...
        $seller = getUser($product->user_id);
        if (!empty($seller) && $this->generalSettings->productbid_system == 1) {

            $emailData = [
                'email_type' => 'bid',
                'email_address' => $seller->email,
                'email_subject' => "New Bid",
                'template_path' => 'email/main',
                'email_data' => serialize([
                    'content' =>"You have a new bid request." . "<br>" ."Bid" . ": <strong>#" . $bidId . "</strong>",
                    'url' => generateDashUrl('productbid_requests'),
                    'buttonText' => trans("view_details")
                ])
            ];
            addToEmailQueue($emailData);
        ...
        $this->session->setFlashdata(
            'product_details_success',"Your bid has been placed successfully. " . $pageLink
        );
        return redirect()->back();
    }

    // Display the  bids with for users
    public function productBids()
    {
        $data = setPageMeta(trans("Bids"));
        $data['activeTab'] = 'productbid_requests';

        $data['numRows'] = $this->biddingModel->getBiddingCount(user()->id);
        $data['pager'] = paginate($this->baseVars->perPage, $data['numRows']);

        $data['bids'] = $this->biddingModel->getBiddingPaginated(
            user()->id,
            $this->baseVars->perPage,
            $data['pager']->offset
        );

        echo view('partials/_header', $data);
        echo view('order/product_bids', $data);
        echo view('partials/_footer');
    }

    // Delete a bid request for the  user
