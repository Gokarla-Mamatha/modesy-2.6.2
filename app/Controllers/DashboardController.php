...
class DashboardController extends BaseController
{
    ...
    public function addProductPost()
    {
        ...
        $title = trim(inputPost('title_' . selectedLangId()) ?? '');
        if (empty($title)) {
    ...
    public function editProductDetailsPost()
    {
        ...
        if (!empty(inputPost('submitted_options_data')) && !empty(inputPost('submitted_variants_data'))) {
            ...
            // echo"<pre>";print_r($variantsToSave);exit;
    ...
    public function sales()
    {
        ...
        $st = inputGet('st');
        $page = ($st == 'completed') ? 'completed_sales' : (($st == 'cancelled') ? 'cancelled_sales' : 'sales');
        $status = ($st == 'completed') ? 'completed' : (($st == 'cancelled') ? 'cancelled' : 'active');
        ...
        $data['pager'] = paginate($this->perPage, $data['numRows']);
        $sales = $this->orderModel->getSalesPaginated($status, $this->userId, $this->perPage, $data['pager']->offset);
        ...
        foreach ($sales as $sale) {
            ...
            if (!empty($item)) {
                $sku = $db->table('product_option_variants')
                    ->select('sku')
        ...
        public function updateOrderProductStatusPost()
    {
        ...
        if (empty($orderProduct) || $this->userId != $orderProduct->seller_id) {
            return redirect()->to(dashboardUrl());
        }
        ...
        if ($this->orderModel->updateOrderProductStatus($id)) {
            ...
            $type        = $orderProduct->product_type;
            $earnings    = new EarningsModel();

            if ($type == 'digital' && in_array($orderStatus, ['completed', 'payment_received'])) {
                $this->orderModel->addDigitalSale($orderProduct->product_id, $orderProduct->order_id);
                $earnings->addSellerEarnings($orderProduct);
            } elseif ($type == 'coupon' && in_array($orderStatus, ['completed', 'payment_received'])) {
                $earnings->addSellerEarnings($orderProduct);
            } elseif ($type == 'physical' && $orderStatus == 'completed') {
                $earnings->addSellerEarnings($orderProduct);
            }

            $orderId = $orderProduct->order_id;
            $items   = $this->orderAdminModel->getOrderItems($orderId);

            $anyCancelled = false;
            $allCompleted = true;

            foreach ($items as $item) {
                if ($item->order_status == 'cancelled') $anyCancelled = true;
                if (!in_array($item->order_status, ['completed', 'cancelled'])) $allCompleted = false;
            }

            $newStatus = $anyCancelled ? 2 : ($allCompleted ? 1 : 0);

            $this->orderAdminModel->builder
                ->where('id', $orderId)
                ->update([
                    'status'     => $newStatus,
    ...
    public function quoteRequests()
    {
        ...
        $data['panelSettings'] = getPanelSettings();
        echo view('dashboard/includes/_header', $data);
        echo view('dashboard/bidding/quote_requests', $data);
        echo view('dashboard/includes/_footer');
    }
    ...
    // Show all product bids for the seller
    public function productBids()
    {
        $data['title'] = trans('Bidding Requests');
        $status = inputGet('status');
        $search = inputGet('q');
        $data['bids'] = $this->biddingModel
            ->getSellerProductBids(user()->id, $status, $search);

        echo view('dashboard/includes/_header', $data);
        echo view('dashboard/bidding/product_bids', $data);
        echo view('dashboard/includes/_footer');
    }

    // Accept the highest bid for a given product
    public function acceptBidPost()
    {
        $productId = inputPost('product_id');

        $bid = $this->biddingModel->acceptBid($productId);

        if ($bid) {

            // send email to bidder
            $bidder = getUser($bid->bidder_id);

            if (!empty($bidder) && $this->generalSettings->productbid_system == 1) {

                $emailData = [
                    'email_type'    => 'bid',
                    'email_address' => $bidder->email,
                    'email_subject' => "Bid Accepted",
                    'template_path' => 'email/main',
                    'email_data'    => serialize([
                        'content' =>
                           "Congratulations! Your bid has been accepted." . "<br>" .
                            trans("Bid") . ": <strong>#" . $bid->id . "</strong>",
                        'url' => generateDashUrl('productbid-requests'),
                        'buttonText' => trans("view_details")
                    ])
                ];
                addToEmailQueue($emailData);
            ...
            setSuccessMessage(trans("Bid accepted successfully"));
        } else {
            setErrorMessage(trans("msg_error"));
        }
    ...
    // Delete a bid request and handle soft delete conditions
    public function deleteBid()
    {
        checkPermission('productbid_requests');

        $id = inputPost('id');

        $this->biddingModel->deleteBid($id);
        $this->biddingModel->deleteBidIfBothDeleted($id);
