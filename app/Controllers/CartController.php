...
class CartController extends BaseController
{
    ...
    public function shipping()
    {
        ...
        if (authCheck()) {
            ...
            if ($isCorrectShippingAddress == false) {
                $data['selectedShippingAddressId'] = 0;
                $stateId = null;
            }

            //reset selected billing address only if invalid
            if ($isCorrectBillingAddress == false) {
                $data['selectedBillingAddressId'] = 0;
            }
            if (empty($data['selectedShippingAddressId'])) {
    ...
    public function shippingPost()
    {
        ...
        $selectedShippingAddress = inputPost('shipping_address_id');

        session()->set('selected_shipping_address', $selectedShippingAddress);
    ...
    public function paymentMethod()
    {
        ...
        if ($checkoutType == 'product') {
            ...
            if (
                ...
                !empty($cart->location_country_id) &&
                !empty($cart->location_state_id)
            ) {
            ...
            $data['cartShippingData'] = !empty($cart->shipping_data)
                ? json_decode($cart->shipping_data)
                : null;

            $data['selectedShippingAddressId'] = 0;

            if (
                !empty($data['cartShippingData']) &&
                !empty($data['cartShippingData']->shippingAddressId)
            ) {
                $data['selectedShippingAddressId'] =
                    $data['cartShippingData']->shippingAddressId;
            }
    ...
    public function payment()
    {
        ...
        if (!empty($cartBackup)) {
            foreach ($cartBackup as $item) {
                $product = $this->productModel->getActiveProduct($item->product_id);
                if (!empty($product)) {
                    $this->cartModel->addToCart(
                        $product,
                        $item->quantity,
                        $item->product_option_variants_id ?? null,
                        $item->extra_options ?? null
                    );
                }
            }
            session()->remove('cart_backup');
        }
    ...
    public function buyNow()
    {
        ...
        $this->cartModel->addToCart($product, $quantity, $variantId, $extraOptions);

        if (
            $product->product_type === 'physical'
            && $this->productSettings->marketplace_shipping == 1
        ) {
            return redirect()->to(base_url('cart/shipping'));
        }

        return redirect()->to(base_url('cart/payment-method'));
    }
        // Add to Cart Bid
    public function addToCartBid()
        {
            $bidId = inputPost('id');
            if (!empty($this->cartModel->addToCartBid($bidId))) {
                return redirect()->to(generateUrl('cart'));
            }
            return redirect()->back();
        }
