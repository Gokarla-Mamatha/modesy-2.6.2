...
class HomeController extends BaseController
{
    ...
    public function initController(\CodeIgniter\HTTP\RequestInterface $request, \CodeIgniter\HTTP\ResponseInterface $response, \Psr\Log\LoggerInterface $logger)
    {
        ...
        $this->blogPerPage = 12;
        $this->forumModel = new ForumCategoryModel();
    }
    ...
    public function index()
    {
        ...
        $data['forumCategories'] = $this->forumModel->getTopCategories();
        $data['trendingProducts'] = $this->productModel->getTrendingProducts(5);
        $data['couponProducts'] = $this->productModel->getCouponProducts($this->activeLang->id);
        ...
        if (empty($product)) {
            ...
            if (!empty($productOptionsData) && !empty($productOptionsData['options']) && countItems($productOptionsData['options']) > 0) {
                ...
                $productOptionsData = $productOptionsModel->getFormattedVariantDataForDetailPage($productOptionsData, $product);
                 if ($product->product_type == 'coupon' && !empty($productOptionsData['variants'])) {
                        foreach ($productOptionsData['variants'] as &$variant) {
                            $variant['is_disabled'] = 
                                $this->productModel->areAllVariantCouponsInactive($variant['id']) ? 1 : 0;
                        }
                    }
            ...
            $data['highestBid'] = $bidModel->getHighestBidByProduct($product->id);
            $data['hasAcceptedBid'] = !empty($bidModel->getAcceptedBidByProduct($product->id));
            //set JSON-LD data
            $jsonLdGenerator = new JsonLdGenerator();
    ...
    public function contactPost()
    {
        ...
        $input = (int) $this->request->getPost('math_captcha');
        $stored = (int) session()->get('math_captcha');
    ...
    // {
    //     log_message('error', 'Controller Hit');

    //     // RESET
    //     if ($this->request->getPost('submit') === 'reset') {

    //         session()->remove('manual_location');

    //         return redirect()->back();
    //     }

    //     // GET VALUES
    //     $countryId   = inputPost('country_id_hidden');
    //     $stateId     = inputPost('state_id_hidden');
    //     $cityId      = inputPost('city_id_hidden');
    //     $locationText = inputPost('location_text');

    //     log_message('error', 'Country ID: ' . $countryId);
    //     log_message('error', 'State ID: ' . $stateId);
    //     log_message('error', 'City ID: ' . $cityId);
    //     log_message('error', 'Location Text: ' . $locationText);

    //     // SAVE SESSION
    //     session()->set('manual_location', [
    ...
    public function setDefaultLocationPost()
    {
        ...
        session()->set('manual_location', [
            'country_id' => $this->request->getPost('country_id_hidden'),
            'state_id' => $this->request->getPost('state_id_hidden'),
            'city_id' => $this->request->getPost('city_id_hidden'),
            'location_text' => $this->request->getPost('location_text')
        ]);
