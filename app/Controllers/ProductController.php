...
class ProductController extends BaseAdminController
{
    ...
    public function saveVariantCoupons()
    {
        $variantKey = $this->request->getPost("variant_key");
        $coupons = json_decode($this->request->getPost("coupons"), true);

        $db = \Config\Database::connect();

        foreach ($coupons as $c) {
            $db->table("variant_coupons")->insert([
    ...
    {
        checkPermission('bidding_products');
        ...
        $numRows = $model->getBiddingCountAdmin($status, $search);
        $data['pager'] = paginate($this->perPage, $numRows);

        $data['products'] = $model->getAdminAllBids(
            $this->perPage,
            $data['pager']->offset,
            $status,
            $search
        ...
        echo view('admin/includes/_header', $data);
        echo view('admin/bidding/product_bids', $data);
        echo view('admin/includes/_footer');
    }
    /**
     * Delete Bidding (Admin)
     */
    public function deleteBidPost()
    {
        checkPermission('bidding_products');
        ...
        if ($model->deleteBiddingAdmin($id)) {
            setSuccessMessage(trans("msg_deleted"));
        } else {
            setErrorMessage(trans("msg_error"));
        }
