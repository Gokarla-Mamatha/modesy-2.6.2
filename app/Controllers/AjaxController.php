...
class AjaxController extends BaseController
{
    ...
    public function addChatPost()
    {
        ...
        $receiverId = intval(inputPost('receiver_id'));
        $data = [
            'result' => 0,
            'senderId' => 0,
            'htmlContent' => ''
    ...
    public function sendMessagePost()
    {
        ...
        if (empty($chat)) {
            return jsonResponse(['status' => 0, 'reason' => 'chat_not_found']);
        }

        if ($chat->sender_id != $userId && $chat->receiver_id != $userId) {
            return jsonResponse(['status' => 0, 'reason' => 'unauthorized']);
        }
    ...
    public function deleteLoyaltyPoints()
    {
        ...
        $db = \Config\Database::connect();
        $db->table('loyalty_point')->where('id', $id)->delete();

        return $this->response->setJSON([
            'status'  => 'success',
            'message' => 'Loyalty points deleted'
        ]);
    }
