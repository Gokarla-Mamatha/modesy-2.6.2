...
class AdminController extends BaseAdminController
{
    ...
    public function initController(\CodeIgniter\HTTP\RequestInterface $request, \CodeIgniter\HTTP\ResponseInterface $response, \Psr\Log\LoggerInterface $logger)
    {
        ...
        $currentPath = service('uri')->getPath();
            ...
            if (!empty($this->generalSettings) && isset($this->generalSettings->auto_approve_orders)) {
    ...
    */
    ...
    public function systemSettingsPost()
    {
        ...
        $digitalProductsSystem = inputPost('digital_products_system');
          $couponProductsSystem = inputPost('coupon_products_system');
        if ($physicalProductsSystem == 0 && $digitalProductsSystem == 0 && $couponProductsSystem == 0) {
    ...
    */
    ...
    //     $data['logs'] = $db->table('user_profile_logs')
    //         ->select('user_profile_logs.*, users.username')
    //         ->join('users', 'users.id = user_profile_logs.user_id')
    //         ->orderBy('user_profile_logs.id', 'DESC')
    //         ->get()
    //         ->getResult();

    //     return view('admin/profile_logs', $data);
    // }
}
