...
class AuthModel extends BaseModel
{
    ...
    public function login()
    {
        ...
        $lastTry = helperGetSession('last_login_try');
        if (is_array($lastTry)) {
            $lastTry = $lastTry['time'] ?? null;
        }

        if ($attempts >= 5 && $lastTry !== null && (time() - $lastTry) < 300) {
   ...
   public function loginWithSocialProvider($provider, $socialUser)
    {
        ...
        if (empty($user)) {
            ...
            if (!empty($referrerId)) {
                ...
                $rewardExists = $this->db->table('loyalty_point_transactions')
                ...
                if (empty($rewardExists)) {
                    ...
                    if ($points > 0) {
                        ...
                        $referrerUser = $this->getUser($referrerId);
                        $newUserObj   = $this->getUser($userId);

                        $this->sendReferralEmail($referrerUser, $newUserObj);
    ...
    public function register()
    {
                ...
                if ($points > 0) {
                    ...
                    if (empty($rewardExists)) {
                        ...
                        if (!$this->db->affectedRows()) {
                            log_message('error', 'REFERRAL INSERT FAILED');
                            log_message('error', json_encode($this->db->error()));
                        }
        ...
        if ($this->generalSettings->email_verification == 1) {
           ...
           session()->set('register_token', $user->token);
           redirectToUrl(generateUrl('register_success'));
        } else {
            $this->loginUser($user);
        }
    ...
    {
        $this->buildQueryUser();
        return $this->builder->where('users.email', trim($email))->get()->getRow();
    }
   ...
   public function filterVendors()
    ...
    {
        return $this->builder
            ->where('referral_code', cleanStr($code))
            ->get()
            ->getRow();
    }

    private function sendReferralEmail($referrer, $newUser)
    {
        if (empty($referrer) || empty($newUser)) {
            return;
        }
        addToEmailQueue([
            'email_type'     => 'referral_reward',
            'email_address'  => $referrer->email,
            'email_subject'  => 'You earned referral points!',
            'email_priority' => 1,
            'template_path'  => 'email/main',
            'email_data'     => serialize([
                'content' => 'Hi ' . esc($referrer->first_name) .
                    ',<br>' .
                    '🎉 You earned referral points because ' .
                    esc($newUser->first_name) .
                    ' joined using your referral link.' .
                    '<br>' .
                    'Keep sharing your referral link to earn more rewards.',
                'buttonText' => 'View Wallet',
                'url' => generateUrl('wallet') . '?tab=loyalty-points'
            ])
        ]);
        addToEmailQueue([
            'email_type'     => 'referred_welcome',
            'email_address'  => $newUser->email,
            'email_subject'  => 'Welcome! You were referred 🎉',
            'email_priority' => 1,
            'template_path'  => 'email/main',
            'email_data'     => serialize([
                'content' =>
                    'Hi ' . esc($newUser->first_name) . ',<br><br>' .
                    'Welcome to Desibazars! 🎉<br>' .
                    'You joined using <strong>' . esc($referrer->first_name) . '</strong>\'s referral link.<br>' .
                    'Start shopping now and earn rewards on every order.',
                'buttonText' => 'Shop Now',
                'url'        => langBaseUrl()
            ])
        ]);
