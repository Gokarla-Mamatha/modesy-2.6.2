...
class ChatModel extends BaseModel
{
    ...
    {
        $receiverId = (int) inputPost('receiver_id');
    ...
    {
        $chat = $this->builder->where('id', $chatId)->get()->getRow();
        ...
        $receiverId = ($chat->sender_id == $userId)
            ? $chat->receiver_id
            : $chat->sender_id;
        $message = esc(trim(inputPost('message')));
        ...
        if ($messageId = $this->builderMessages->insert($data)) {
            $this->builder->where('id', clrNum($chatId))
    ...
    {
        return $this->builderMessages
            ->join('chat', 'chat.id = chat_messages.chat_id')
            ->where('chat_messages.receiver_id', $userId)
            ->where('chat_messages.is_read', 0)
            ->where('chat_messages.deleted_user_id', 0)
            ->groupStart()
                ->where('chat.sender_id', $userId)
                ->orWhere('chat.receiver_id', $userId)
            ->groupEnd()
            ->countAllResults();
    }
