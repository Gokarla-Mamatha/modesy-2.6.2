...
class CheckoutModel extends BaseModel
{
    ...
    private function createCheckoutFromCart(object $cart): ?object
    {
        ...
        foreach ($cart->items as $item) {
            ...
            $checkoutItems[] = [
                ...
                'product_option_variants_id' => $item->product_option_variants_id,
    ...
    {
        return $this->builderCheckoutItems
        ->select('checkout_items.*, checkout_items.product_option_variants_id')
        ->where('checkout_id', $checkoutId)->get()->getResult();
    }
...
$isBuyNow = session()->get('is_buy_now_checkout');

if (!$isBuyNow) {
    // ✅ Normal cart checkout → clear cart
    $cartModel->clearCart($checkout);
}

// ✅ Cleanup flag
session()->remove('is_buy_now_checkout');
