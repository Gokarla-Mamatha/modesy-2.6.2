...
class ProductOptionsModel extends Model
{
        ...
        foreach ($variantsResult as $variantRow) {
            ...
            if (!empty($variantComposition['value_ids'])) {
                ...
                $couponRows = $this->db->table('variant_coupons')
                    ->where('product_option_variants_id', $variantRow->id)
                    ->orderBy('id', 'ASC')
                    ->get()
                    ->getResultArray();
                $variantsData[] = [
    ...
    int $productId,
    array $variantsToSave,
    array $clientToServerValueIdMap,
    array $clientValueToOptionKeyMap,
    array $clientValueToValueKeyMap
): void {

    $existingVariantsResult = $this->db->table('product_option_variants')
        ->select('id')
        ->where('product_id', $productId)
        ->get()
        ->getResultArray();
    ...
    if (!empty($existingVariantsResult)) {
        ...
        $this->db->table('variant_coupons')
            ->whereIn('product_option_variants_id', $existingVariantServerIds)
            ->delete();

        $this->db->table('product_option_variant_values')
            ->whereIn('variant_id', $existingVariantServerIds)
            ->delete();
    }

        $this->db->table('product_option_variants')
            ->where('product_id', $productId)
            ->delete();
    ...
    if (is_array($variantsToSave) && !empty($variantsToSave)) {
        ...
        foreach ($variantsToSave as $variantData) {
            ...
            $variantDbPayload = [
                ...
                'is_default' => (!empty($variantData['is_default']) && $variantData['is_default'] == 1) ? 1 : 0,
                'is_active' => (!empty($variantData['is_active']) && $variantData['is_active'] == 1) ? 1 : 0,
            ...
            if (!empty($variantData['coupons'])) {
                    $this->db->table('variant_coupons')
                        ->where('product_option_variants_id', $newVariantId)
                        ->delete();
                        if (!empty($variantData['coupons'])) {
                        $batchData = [];
                        foreach ($variantData['coupons'] as $coupon) {
                            $batchData[] = [
                                'product_option_variants_id' => $newVariantId,
                        ...
                        if (!empty($batchData)) {
                            $this->db->table('variant_coupons')->insertBatch($batchData);
                        }
                    }
            }
            if ($newVariantId > 0 && is_array($variantData['composition'])) {
                foreach ($variantData['composition'] as $compositionValue) {
                    $dbValueId = $clientToServerValueIdMap[$compositionValue['value_client_id']] ??
                                ($compositionValue['value_server_id'] ?? null);
