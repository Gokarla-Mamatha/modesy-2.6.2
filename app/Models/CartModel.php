...
class CartModel extends BaseModel
{
    ...
    public function addToCart($product, $quantity, $variantId = null, $extraOptions = [])
    {
        ...
        // Build a stable unique cart item hash
        $itemHash = md5($product->id . $variantId . $extraOptionsHash);

        // If same item already exists in cart, increase quantity
        $existingItem = $this->builderCartItems
        ...
        $cartItem = [
            ...
            'quote_request_id' => 0,
            'variant_hash' => !empty($variant) ? $variant->variant_hash : null,
            ...
            'product_options_snapshot' =>
                !empty($variantId) ? 
                $productOptionsModel->getVariantSnapshot($variantId, $extraOptions) : ''
        ];
    ...
    public function addToCartQuote($quoteRequestId)
    {
        ...
        $cartItem = [
            ...
            'item_hash' => '',
            'product_id' => $product->id,
            'product_option_variants_id' => $quoteRequest->product_option_variants_id ?? null,
    ...
    private function syncCartItem($cart, $cartItem, $product, $includeTaxes)
    {
        ...
        // PRODUCT BID (auction) – accepted bid
       if ($cartItem->purchase_type == 'productbid') {

        $bid = $this->db->table('bids')
            ->select('bid_amount')
            ->where('id', $cartItem->bid_id)
            ->where('status', 'accepted')
            ->get()
            ->getRow();

        if (!empty($bid)) {
            $unitPrice = $bid->bid_amount;
        ...
        if (
            $this->paymentSettings->currency_converter == 1 &&
            !empty($cart->currency_code) &&
            !empty($cart->currency_exchange_rate)
        ) {
            $unitPrice = convertCurrencyByExchangeRate($unitPrice, $cart->currency_exchange_rate);
            $totalPrice = convertCurrencyByExchangeRate($totalPrice, $cart->currency_exchange_rate);
        }

        // QUOTE / EXISTING BIDDING SYSTEM
        } elseif ($cartItem->purchase_type == 'bidding') {
            ...
            // NORMAL PRODUCTS ONLY
                ...
                if (!empty($variant)) {
                    $variantPrice = (!empty($variant->price_discounted) && $variant->price_discounted > 0)
                        ? $variant->price_discounted
                        : $variant->price;
    ...
    //Add to cart bid
   public function addToCartBid($bidId)
{
    $biddingModel = new BiddingModel();
    $bid = $biddingModel->getBid($bidId);

    // must be accepted bid
    if (empty($bid) || $bid->status !== 'accepted') {
        return false;
    }

    $product = getActiveProduct($bid->product_id);
    if (empty($product)) {
        return false;
    }
    ...
    // 🔑 THIS IS THE FIX
    $unitPrice = numToDecimal($bid->bid_amount);
    $totalPrice = $unitPrice;
    ...
    $cartItem = [
        ...
        'product_id'       => $product->id,
        'quantity'         => 1,
        'purchase_type'    => 'productbid',
        'bid_id'           => $bidId, 
        'product_type'     => $product->product_type,
        'listing_type'     => $product->listing_type,
        'product_title'    => $product->title,
        'unit_price_base'  => $unitPrice,   // ✅ bid_amount
        'unit_price'       => $unitPrice,   // ✅ bid_amount
        'total_price'      => $totalPrice,  // ✅ bid_amount
        'seller_id'        => $product->user_id,
        'is_stock_available' => 1,
        'created_at'       => date('Y-m-d H:i:s')
    ];
