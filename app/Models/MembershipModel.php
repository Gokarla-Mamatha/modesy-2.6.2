...
class MembershipModel extends BaseModel
{
    ...
    public function preparePlanData()
    {
        $data = [
            ...
            'storage_limit' => inputPost('storage_limit') ?? 0
        ];
        $arrayTitle = array();
        $arrayFeatures = array();
    ...
    public function getReferrerUsers()
    {
        return $this->db->table('users u')
            ->select("u.*, CONCAT(r.first_name, ' ', r.last_name) AS ref_name")
            ->join('users r', 'r.id = u.referred_by', 'left')
            ->orderBy('u.id', 'DESC')
            ->get()
            ->getResult();
    }
