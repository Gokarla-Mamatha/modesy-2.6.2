...
class OrderModel extends BaseModel
{
    ...
    //add order items
  public function addOrderItems(int $orderId, object $checkout, array $checkoutItems, string $orderStatus): array|bool
    {
        ...
        // decode seller-wise shipping once
        $cartShippingMethods = safeJsonDecode($checkout->shipping_cost_data);

        foreach ($checkoutItems as $item) {

            $variantId = $item->product_option_variants_id ?? null;
            if ($item->product_type === 'coupon' && empty($variantId)) {
                throw new \Exception("Coupon product requires a variant ID.");
            }
            ...
            $data = [
                ...
                'product_option_variants_id' => $variantId,
                ...
                'product_total_price' => numToDecimal($item->total_price) + numToDecimal($item->product_vat),
                'created_at' => date('Y-m-d H:i:s'),
                'updated_at' => date('Y-m-d H:i:s'),
            ];

            // add seller shipping ONLY for physical products
            if ($item->product_type !== 'digital' && !empty($cartShippingMethods)) {
                foreach ($cartShippingMethods as $method) {
                    if ($method->shop_id == $item->seller_id) {
                        $data['shipping_method'] = $method->shipping_method;
                        $data['seller_shipping_cost'] = numToDecimal($method->shipping_cost);
                        break;
                    }
                }
            }

            // insert order item (ONLY ONCE)
            $this->builderOrderItems->insert($data);
            ...
            if ($item->product_type === 'coupon') {
                ...
                $variant = $this->db->table('product_option_variants')
                    ->select('is_default')
                    ->where('id', $variantId)
                    ->get()
                    ->getRow();

                if (!empty($variant) && (int) $variant->is_default === 1) {
                    $orderCoupons[$orderItemId] = ['DEFAULT_COUPON'];
                } else {
                    $couponRows = $this->db->table('variant_coupons')
                    ...
                    if (count($couponRows) < $qty) {
                        throw new \Exception("Not enough coupons available");
                    }

                    $couponIds   = array_column($couponRows, 'id');
                    $couponCodes = array_column($couponRows, 'coupon_code');

                    $this->db->table('variant_coupons')
                    ...
                    $orderCoupons[$orderItemId] = $couponCodes;
                }
            }

            // digital stock update preparation
            if ($item->product_type === 'digital') {
        ...
        // update digital product stock
        if (!empty($productsToUpdateStock)) {
            $this->db->table('products')
                ->whereIn('id', $productsToUpdateStock)
                ->update(['is_sold' => 1]);
        }
        ...
        if ($this->db->transStatus() === false) {
            log_message('error', '[OrderModel] Failed to create order items (Order ID: ' . $orderId . ')');
            return false;
        }

        (new CommonModel())->deleteAffiliateCookie($productIdsForAffiliate);
   ...
   public function getOrderItemCoupons($orderItemId)
    {
    ...
    public function processOrderCompletion(int $orderId): bool
    {
        if ($orderId <= 0) {
            return false;
        }
        ...
        if ((int)$order->loyalty_given == 1) return false;
        if ((int)$order->buyer_id <= 0) return false;

        $totalPoints = 0;
        $items = $this->db->table('order_items oi')
            ->select('oi.product_quantity, p.loyalty_points')
        ...
        if (!empty($items)) {
            foreach ($items as $item) {
                $points = (int) ($item->loyalty_points ?? 0);
                $qty    = (int) ($item->product_quantity ?? 1);
                $totalPoints += ($points * $qty);
            }
        }

        $orderAmount = $order->price_total ?? 0;

        if ($orderAmount > 0) {
            $rule = $this->db->table('loyalty_point')
                ->where('status', 1)
            ...
            if ($rule) {
                $totalPoints += (int) $rule->points;
            }
        }
        if ($totalPoints <= 0) {
            $updated = $this->db->table('orders')
                ->where('id', $orderId)
                ->update(['loyalty_given' => 1]);

            return true;
        }
        // update user
        $this->db->table('users')
            ->set('loyalty_points', 'loyalty_points + ' . $totalPoints, false)
