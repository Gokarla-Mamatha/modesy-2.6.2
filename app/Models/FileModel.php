...
class FileModel extends BaseModel
{
    ...
    public function addProductImages($productId): bool
    {
        try {
            ...
            $plan = $this->db->table('users_membership_plans')
            ...
            if (!empty($plan) && (int)$plan->storage_limit > 0) {

                $newMB = 0;
                foreach ($images as $image) {
                    $temp = FCPATH . 'uploads/temp/' . $image->image_big;
                    if (file_exists($temp)) {
                        $newMB += ceil(filesize($temp) / (1024 * 1024));
                    }
                }

                $user = getUser($userId);
                $usedMB = (int)($user->storage_used ?? 0);

                if (($usedMB + $newMB) > (int)$plan->storage_limit) {
                    setErrorMessage('Storage limit reached. Upgrade your membership.');
                    helperDeleteSession('mds_product_images');
                    return false;
                }
            }
            $dateFolder = $yearMonth;
            $baseDirectory = 'uploads/images/' . $userId . '/' . $dateFolder . '/';

            if (!is_dir(FCPATH . $baseDirectory)) {
                mkdir(FCPATH . $baseDirectory, 0777, true);
            }
            ...
            foreach ($images as $image) {
                ...
                $data = [
                    'product_id'    => $productId,
                    'user_id'       => $userId, 
                    'image_small' => $userId . '/' . $dateFolder . '/' . $image->image_small,
                    'image_default' => $userId . '/' . $dateFolder . '/' . $image->image_default,
                    'image_big'     => $userId . '/' . $dateFolder . '/' . $image->image_big,
            ...
            // 6️⃣ Update used storage
            if (!empty($plan) && (int)$plan->storage_limit > 0) {
                $this->db->table('users')
                    ->where('id', $userId)
                    ->set('storage_used', 'storage_used + ' . $newMB, false)
                    ->update();
            }
