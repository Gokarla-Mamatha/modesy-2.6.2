...
class EarningsModel extends BaseModel
{
    ...
    // Earned points
    public function getEarnedLoyaltyPaginated($userId, $limit, $offset)
    {
    ...
    public function getEarnedLoyaltyCount($userId)
    {
        return $this->db->table('loyalty_point_transactions')
            ->where('user_id', $userId)
            ->where('type', 'earned')
    ...
    public function getRedeemLoyaltyPaginated($userId, $limit, $offset)
    {
        return $this->db->table('loyalty_point_transactions')
            ->where('user_id', $userId)
            ->where('type', 'redeem')
    ...
    public function getRedeemLoyaltyCount($userId)
    {
        return $this->db->table('loyalty_point_transactions')
            ->where('user_id', $userId)
            ->where('type', 'redeem')
    ...
    {
        $row = $this->db->table('loyalty_point_transactions')
            ->selectSum('balance')
            ->where('user_id', $userId)
            ->where('type', 'earned')
            ->get()
    ...
    public function getLoyaltySettings()
        {
            return $this->db->table('settings')
                ->select('loyalty_convert_points, loyalty_convert_amount')
                ->where('id', 1)
                ->get()
                ->getRow();
        }
