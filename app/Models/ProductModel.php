...
class ProductModel extends BaseModel
{
    ...
    public function addProduct()
    {
        helper(['security', 'text']);
         ...
         $productType = strip_tags(inputPost('product_type'));
         $listingType = strip_tags(inputPost('listing_type'));
         $biddingStartRaw = inputPost('bidding_start_time');
         $biddingEndRaw   = inputPost('bidding_end_time');

         $biddingStart = !empty($biddingStartRaw)
            ? date('Y-m-d H:i:s', strtotime($biddingStartRaw))
            : null;

         $biddingEnd = !empty($biddingEndRaw)
            ? date('Y-m-d H:i:s', strtotime($biddingEndRaw))
            : null;

        $slug = url_title(inputPost('title_' . defaultLangId()), '-', true);

        $data = [
            'slug' => substr(preg_replace('/[^a-z0-9-]/', '', $slug), 0, 200),
            ...
            'rating' => 0,
            'pageviews' => 0,
            'demo_url' => filter_var(inputPost('demo_url'), FILTER_SANITIZE_URL),
            'external_link' => filter_var(inputPost('external_link'), FILTER_SANITIZE_URL),
            'files_included' => strip_tags(inputPost('files_included')),
            'stock' => 1,
            'shipping_delivery_time_id' => 0,
            'multiple_sale' => 1,
            'digital_file_download_link' => filter_var(inputPost('digital_file_download_link'), FILTER_SANITIZE_URL),
            'is_deleted' => 0,
            'is_draft' => 1,
            'is_free_product' => 0,
            'country_id' => 0,
            ...
            'bidding_status' => ($listingType === 'productbid') ? 'active' : 'closed',
            'bidding_start_time' => $biddingStart,
            'bidding_end_time' => $biddingEnd,
            'minimum_price' => 0,
            'bid_quantity' => (int) (inputPost('bid_quantity') ?? 1),
            'accepted_bid_id' => null,
            'loyalty_points' => (int) (inputPost('loyalty_points') ?? 0)
        ];
        ...
        if ($productType === 'coupon') {
            ...
            $data['stock'] = 0;
            $data['coupon_redeem_place'] = strip_tags(inputPost('coupon_redeem_place'));
            $data['coupon_contact'] = strip_tags(inputPost('coupon_contact'));
            $data['coupon_map_link'] = filter_var(inputPost('coupon_map_link'), FILTER_SANITIZE_URL);
            $data['coupon_expiry'] = !empty(inputPost('coupon_expiry'))
                ? date("Y-m-d H:i:s", strtotime(inputPost('coupon_expiry')))
                : null;
            $data['coupon_description'] = strip_tags(inputPost('coupon_description'));
        }
        if (empty($data['sku'])) {
            $data['sku'] = '';
        }
        ...
        $data['category_id'] = (int) getDropdownCategoryId();
    ...
    public function addProductTitleDesc($productId)
    {
        ...
        // fallback title
        $mainTitle = strip_tags(trim(inputPost('title_' . defaultLangId())));

        // load TagModel properly
        $tagModel = new \App\Models\TagModel();

        foreach ($this->activeLanguages as $language) {

            $titleInput = trim(inputPost('title_' . $language->id));
            $title = !empty($titleInput)
                ? strip_tags($titleInput)
                : $mainTitle;
            ...
            // ✅ SAFE DESCRIPTION (instead of HTMLPurifier)
            $descriptionRaw = inputPost('description_' . $language->id) ?? '';
            $description = strip_tags(
                $descriptionRaw,
                '<p><br><b><strong><i><em><u><ul><ol><li><h1><h2><h3><h4><h5><h6><blockquote><a>'
            );

            $shortDescription = strip_tags(
                trim(inputPost('short_description_' . $language->id))
            );

            // insert data
            $this->builderProductDetails->insert([
    ...
    {
        helper(['security', 'product']);

        $mainTitle = esc(trim(inputPost('title_' . defaultLangId())));

        foreach ($this->activeLanguages as $language) {

            $title = strip_tags(trim(inputPost('title_' . $language->id)));

            $rawDescription = inputPost('description_' . $language->id) ?? '';
            ...
            $data = [
                ...
                'description'       => $description,
                'short_description' => strip_tags(trim(inputPost('short_description_' . $language->id)))
            ];
            ...
            if (empty($row)) {
                ...
                $this->builderProductDetails
                    ->where('product_id', clrNum($productId))
    ...
    public function editProductDetails($id)
    {
        ...
        $data = [
            ...
            'coupon_redeem_place'    => inputPost('coupon_redeem_place'),
            'coupon_contact'         => inputPost('coupon_contact'),
            'coupon_map_link'        => inputPost('coupon_map_link'),
            'coupon_description'     => inputPost('coupon_description'),
            'coupon_expiry'          => inputPost('coupon_expiry'),
            'bidding_status'       => inputPost('bidding_status'),
            'bidding_end_time'      => inputPost('bidding_end_time'),
            'accepted_bid_id'      => inputPost('accepted_bid_id'),
            'bidding_start_time'    => inputPost('bidding_start_time'),
            'minimum_price'         => numToDecimal(inputPost('minimum_price') ?? 0),
            'bid_quantity'          => clrNum(inputPost('bid_quantity')),
            'loyalty_points' => clrNum(inputPost('loyalty_points') ?? 0),
    ...
    public function getCouponProducts()
    {
        ...
        return $this->builder
            ->where('products.product_type', 'coupon')
            ->orderBy('products.created_at', 'DESC')
            ->get()
            ->getResult();  
    }

    public function areAllVariantCouponsInactive($variantId)
    {
        $db = \Config\Database::connect();

        $total = $db->table('variant_coupons')
            ->where('product_option_variants_id', $variantId)
            ->countAllResults();

        $active = $db->table('variant_coupons')
            ->where('product_option_variants_id', $variantId)
            ->where('status', 'active')
            ->countAllResults();
    ...
    public function getActiveCouponCountByVariant($variantId)
    {
        return $this->db->table('variant_coupons')
            ->where('product_option_variants_id', $variantId)
            ->where('status', 'active')
            ->countAllResults();
    }
    public function getTrendingProducts($langId)
    {
        $cacheKey = 'trending_products_lang_' . $langId;
        ...
        return getCacheData($cacheKey, function () {
            ...
            return $this->builder
                ...
                ->orderBy('products.pageviews', 'DESC')
                ->limit(5)
                ->get()
                ->getResult();
        }, 'product');
