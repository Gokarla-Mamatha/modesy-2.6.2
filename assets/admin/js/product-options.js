$(document).ready(function () {
    ...
    function addOptionGroup(optionData = null, isButtonClick = false) {
        ...
        const optionHtml = `
            <div class="option-group" id="${clientOptionId}"
                 ...
                 data-translations='${encodeURIComponent(JSON.stringify(initialTranslations))}'
    ...
    function createOptionValueEntryHtml(currentOptionType, valueClientId, valueServerId, valueKey, nameTranslations = {}, colorValue = '#ffffff', imageObjects = [], primarySwatchObject = null) {
        ...
        return `
            <div class="option-value-entry" data-value-client-id="${valueClientId}" ${serverIdAttribute} ${valueKeyAttribute}
                 data-image-objects='${encodeURIComponent(JSON.stringify(imageObjects))}'
                data-primary-swatch='${encodeURIComponent(JSON.stringify(primarySwatchObject))}'
                data-translations='${encodeURIComponent(JSON.stringify(nameTranslations))}'>
    ...
    $('#options-container').on('change', '.option-type-selector', function () {
        ...
        const proceedWithChange = (confirmedNewType) => {
            ...
            $($valueEntriesToReformat).each(function () {
                ...
                try {
                    nameTranslations = JSON.parse(decodeURIComponent($oldValueEntry.attr('data-translations') || '%7B%7D'));
    ...
    function handleDirectNameInput() {
        ...
        let translations = {};
        try {
            translations = JSON.parse(decodeURIComponent($targetElement.attr('data-translations') || '%7B%7D'));
        } catch (e) {
        ...
        $targetElement.attr('data-translations',encodeURIComponent(JSON.stringify(translations)));
    ...
    $('#options-container').on('click', '.btn-translate', function () {
        ...
        try {
            currentTranslations = JSON.parse(decodeURIComponent(currentTranslationTargetElement.attr('data-translations') || '%7B%7D'));
    ...
    $('#saveTranslationsBtn').on('click', function () {
        ...
        currentTranslationTargetElement.attr('data-translations',encodeURIComponent(JSON.stringify(newTranslations)));
    ...
    function updateValueEntryVisuals(optionValueEntry) {
        ...
        let primarySwatchObject = null;
        try {
           imageObjects = JSON.parse(decodeURIComponent($entry.attr('data-image-objects') || '%5B%5D'));
            if (!Array.isArray(imageObjects)) imageObjects = [];
        } catch (e) {
            imageObjects = [];
        ...
        try {
            primarySwatchObject = JSON.parse(decodeURIComponent($entry.attr('data-primary-swatch') || 'null'));
        } catch (e) {
    ...
    $('#saveOptionImagesButton').on('click', function () {
        ...
        if (managementMode === 'single-swatch-image') {
            ...
            currentManagingOptionValueEntry.attr('data-primary-swatch',encodeURIComponent(JSON.stringify(newSwatchObject)));
        } else {
            currentManagingOptionValueEntry.attr('data-image-objects',encodeURIComponent(JSON.stringify(tempOptionImages)));        
        }
    ...
    function getOptionsData() {
        ...
        $('#options-container .option-group').each(function (index) {
            ...
            try {
                optionNameTranslations = JSON.parse(decodeURIComponent($optionGroup.attr('data-translations') || '%7B%7D'));
            ...
            if (optionType !== 'text' && optionType !== 'number' && optionType !== 'checkbox') {
                $optionGroup.find('.option-values-container .option-value-entry').each(function () {
                    ...
                    try {
                        valueNameTranslations = JSON.parse(decodeURIComponent($valueEntry.attr('data-translations') || '%7B%7D'));
                    } catch (e) {
                    ...
                    let imageObjects = [];
                    try {
                        imageObjects = JSON.parse(decodeURIComponent($valueEntry.attr('data-image-objects') || '%5B%5D'));
                    ...
                    try {
                        primarySwatch = JSON.parse(decodeURIComponent($valueEntry.attr('data-primary-swatch') || 'null'));
    ...
    function updateVariants(forceSkuRegeneration = false) {
        ...
        newVariants.forEach(variant => {
            ...
            finalVariantData.coupons = oldData.coupons || [];
            ...
            const couponButtonHtml = `
                <button type="button" 
                        class="btn btn-xs btn-info open-coupon-btn"
                        data-stable-key="${finalVariantData.stable_key}"
                        data-variant-qty="${quantityValue}">
            ...
            <tr data-stable-key="${finalVariantData.stable_key}" class="${rowClass}">
                ...
                <td style="width: 6%;">${couponButtonHtml}</td>
    ...
    function prepareDataForSubmission() {
        ...
        $('#variants-table-body').find('tr[data-stable-key]').each(function () {
            ...
            if (variantKey && previousVariantData[variantKey]) {
                ...
                if (!previousVariantData[variantKey].is_default) {
                    ...
                    previousVariantData[variantKey].coupons = previousVariantData[variantKey].coupons || [];
        ...
        $('#options-container .option-group').each(function (optionIndex) {
            ...
            const optionData = {
                ...
                name_translations: JSON.parse(decodeURIComponent($optionGroup.attr('data-translations') || '%7B%7D')),
            ...
            $optionGroup.find('.option-values-container .option-value-entry').each(function (valueIndex) {
                ...
                optionData.values.push({
                    ...
                    name_translations: JSON.parse( decodeURIComponent($valueEntry.attr('data-translations') || '%7B%7D') ),
...
$(document).on("click", ".open-coupon-btn", function () {

    const key = $(this).data("stable-key");
    const row = $(this).closest("tr");
    const qty = parseInt(row.find(".variant-quantity").val()) || 0;
    const variantName = escapeHtml(previousVariantData[key].name);

    $("#couponModal").data("variant-key", key);
    $("#couponRowsContainer").html("");

    let existingCoupons = previousVariantData[key].coupons || [];

    for (let i = 0; i < qty; i++) {

        let c = existingCoupons[i] || {
            coupon_code: "",
            status: "active"
        };

        $("#couponRowsContainer").append(`
            <div class="row m-b-15 coupon-row">
                <div class="col-md-3">
                    <strong>${variantName} :- ${i + 1}</strong>
                </div>
                <div class="col-md-3">
                    <select class="form-control coupon-status">
                  ...
                  <div class="col-md-6">
                    <div class="input-group">
                        <input type="text" class="form-control coupon-code" value="${c.coupon_code}" placeholder="Enter coupon code"required>
                        <span class="input-group-btn"><button class="btn btn-primary generate-code-btn" type="button">Generate</button></span>
                    </div>
                </div>
...
$(document).on("click", ".generate-code-btn", function () {
    ...
    $(this).closest(".input-group").find(".coupon-code").val(code);
});


$(document).on("click", "#saveCouponsBtn", function () {

    let key = $("#couponModal").data("variant-key");
    let coupons = [];
    let isValid = true;
    $("#couponRowsContainer .coupon-row").each(function () {
        const status = $(this).find(".coupon-status").val();
        const couponInput = $(this).find(".coupon-code");
        const coupon_code = couponInput.val().trim();
        if (coupon_code === "") {
    ...
    if (!isValid) {
        alert("Please enter coupon code for all variants.");
        return;
    }
    previousVariantData[key].coupons = coupons;
